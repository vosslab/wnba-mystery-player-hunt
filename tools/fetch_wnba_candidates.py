...
def index_fantasy_points(rows: list[dict], season: str) -> dict[str, float | int]:
	...
	for row in rows:
		...
		points = require_field(row, "NBA_FANTASY_PTS", f"{season} player {player_id}")
		if not isinstance(points, (int, float)) or isinstance(points, bool):
			raise ValueError(f"{season} player {player_id} has nonnumeric NBA_FANTASY_PTS")
...
def build_candidates(sources: dict) -> dict:
	...
	for roster_source in rosters:
		...
		for roster_row in roster_rows:
			...
			if player_id not in previous_points:
			...
			candidate = {
				...
				"fantasyPointsPreviousSeason": previous_points[player_id],
			}
	...
	output = {
		...
		"source": {"kind": "official-wnba-stats", "urls": sources_metadata},
		"validation": {
